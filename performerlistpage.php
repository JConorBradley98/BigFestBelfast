...
if (isset($_POST["performers"])) {
    $performerspage = $_POST["performers"];
} else {
    $performerspage = 1;
};
$performerstart = ($performerspage - 1) * $performerresults;
$userread = "SELECT * FROM user WHERE usertype ='performer' LIMIT " . "$performerstart" . " , " . "$performerresults";


$userresult = $conn->query($userread);

$performercount = "SELECT COUNT(ID) AS total FROM user WHERE usertype = 'performer'";
$countresult = $conn->query($performercount);
...
$totalperformerpages = ceil($totalrows / $performerresults);
?>
...
?>
                ...
                <?php
                for ($i = 1; $i <= $totalperformerpages; $i++) {
                    echo "<div style='padding-left:50px; padding=right:50px'> <a href='performerlistpage.php?performers=" . $i . "'> <button type ='button' class = 'btn btn-danger'> $i </button> </a> </div> ";
                }
                ?>
