...
$schedulestart = ($schedulepage - 1) * $scheduleresults;
...
if(isset($_SESSION['venue'])){
    ...
    $schedulecount = "SELECT COUNT(ID) AS total FROM showdb WHERE venueid = $venueid";
    $scheduleread = "SELECT * FROM showdb INNER JOIN showpage ON showdb.id = showpage.showid WHERE showdb.venueid = $venueid  LIMIT " . "$schedulestart" . " , " . "$scheduleresults";
}

if(isset($_SESSION['performer'])){
    $performerid = $_SESSION['performer'];
    $user = $_SESSION['performer'];
    $schedulecount = "SELECT COUNT(showdb.ID) AS total FROM showdb INNER JOIN showpage ON showdb.id = showpage.showid WHERE performerid = $performerid";
    $scheduleread = "SELECT * FROM showdb INNER JOIN showpage ON showdb.id = showpage.showid WHERE showdb.performerid = $performerid  LIMIT " . "$schedulestart" . " , " . "$scheduleresults";
}



$scheduleresult = $conn->query($scheduleread);


$schedulecountresults = $conn->query($schedulecount);
$schedulerow = $schedulecountresults->fetch_assoc();
$totalrows = $schedulerow['total'];




$totalschedulepages = ceil($totalrows / $scheduleresults);
?>
...
<div class = "col-xl-12 btn-group" style="text-align:center;">
                ...
                <?php
                for ($i = 1; $i <= $totalschedulepages; $i++) {
                    echo "<div class ='col-sm-4 text-center' style='padding-left:30px; padding=right:30px'> <a href='scheduleview.php?schedule=" . $i . "'> <button type ='button' class = 'btn btn-danger'> $i </button> </a>  </div> ";
                }
                ?>
