...
$venuefindquery = "SELECT * FROM user WHERE usertype='venue' AND `venue` = 1";

$excecutevenuefindquery = $conn->query($venuefindquery);
...
if ($check == 1) {
    ...
    if (empty($_POST['venue'])) {
        echo"Venue Does not Exist";
        $confirmationechocheck = 0;
        die();
    }

    if (empty($_POST['date'])) {
        echo "No start date Selected";
        $confirmationechocheck = 0;
        die();
    }
    ...
    $venuevalidationquery = $conn->prepare("SELECT * FROM venue INNER JOIN user ON venue.managerid = user.id WHERE user.username=? AND user.venue = 1");
    $venuevalidationquery->bind_param("s", $_POST['venue']);
    ...
    if ($venuevalidationqueryresult->num_rows > 0) {
        ...
        $venueuserjoin = $conn->prepare("SELECT venue.id, venue.managerid, user.username FROM venue INNER JOIN user ON venue.managerid = user.id WHERE user.username = ?");
        $venueuserjoin->bind_param("s", $_POST['venue']);
        $venueuserjoin->execute();
        $venueuserjoinresult = $venueuserjoin->get_result();
        $venueuserjoinrow = $venueuserjoinresult->fetch_assoc();

        $managerid = $venueuserjoinrow['managerid'];
        ...
        if($timeslotcheckqueryresults->num_rows  > 0){
         ...
         echo "You already have a booking at this time";
         $confirmationechocheck = 0;
         die();
         }
            ...
            <div class="col-xl-12 text-center" id="con">
                ...
                <form action="application.php" method="post">
                    <div class="form-group-inline text-center" id="loggroup">
                        <h1 class="formh"> Send Application </h1>
                        <?php
                        ...
                        <div class="row justify-content-center">
                            ...
                            <label>   <h1 class="footertext"> Desired Venue   </h1> </label>
                                <select class="custom-select custom-select-lg mb-3" name = 'venue' required>
                                    <option selected value=""> Select venue</option>
<?php
while ($row = $excecutevenuefindquery->fetch_assoc()) {
    $recipientname = $row['username'];



    echo "<option value ='$recipientname'> $recipientname </option>";
}
?>
                                </select>
                            </div>
                        ...
                        <div class="row justify-content-center">
                            ...
                            <label>   <h1 class="footertext"> Start Time   </h1> </label>
                                <input class = "form-control"  id="logcon" type = "datetime-local"  min="2020-06-29T00:00" max ="2020-07-05T23:59" name="date" placeholder="Start Date" required>
                            </div>
                        </div>
                        <div class="row justify-content-center">
                            <div class="logdiv center-block">
